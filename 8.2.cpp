...
Problem
	...
	Design an algorithm to get all possible paths for therobot
*/
/*
Solution
	Part 1:(For clarity, we will solve this part assuming an X by Y grid)
	Each path has (X-1)+(Y-1) steps 
	...
	That is, if I were to ask you which path you took, you could simply say ¡°I moved right on step 3 and 4 ¡±
	Since you must always move right X-1 times, and you have X-1 + Y-1 total steps, you have to pick X-1 times to move right out of X-1+Y-1 choices 
...
void PathCount(int x, int y, int& nPathCnt)
{
	if (1 == x || 1 == y)
	{
		++nPathCnt;
	...
	{
		PathCount(x - 1, y, nPathCnt);
		PathCount(x, y - 1, nPathCnt);
	}
}
