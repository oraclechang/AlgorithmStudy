...
Solution
	...
	That is, for the first element, there are 2 choices 
	For the second, there are two, etc So, doing 2 * 2 * ... * 2 n times gives us 2^n subsets 
	We will not be able to do better than this in time or space complexity 

	Approach #1: Recursion
	This is a great problem to implement with recursion since we can build all subsets of a set using all subsets of a smaller set Specifically, 
	given a set S, we can do the following recursively:
...
/*
	...
	- When we¡¯re generating a set, we have two choices for each element: (1) the element is in the set (the ¡°yes¡± state) or (2) the element is not in the set (the ¡°no¡± state) 
	This means that each subset is a sequence of yesses / nos - eg , ¡°yes, yes, no, no, yes, no¡±
...
deque< deque< int > > GetSubsets(deque< int > set, int nIdx)
{
	deque< deque< int > > AllSubsets;

	if (set.size() == nIdx)
		AllSubsets.push_back(deque< int >());
	else
	{
		AllSubsets = GetSubsets(set, nIdx + 1);
		int item = set[nIdx];

		deque< deque< int > > moreSubsets;

		for (auto subset : AllSubsets)
		{
			deque< int > newSubset(subset);
